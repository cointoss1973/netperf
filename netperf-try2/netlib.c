...
int	tcp_proto_num;	/* this would appear unitialized (pavel 4-Feb-1998) */
...
/* The NSIGS below is a portability hazard.  POSIX does not specify NSIGS, so
   ...
   backward compat), although Solaris advises that "for binary compatibility,
   one should use _sys_nsig instead". Insert just a minimal kluge for VxWorks
   (pavel 7-Jan-1998) */
#ifdef VXWORKS
#define NSIG _NSIGS
#endif
...
if (bytes_recvd == 0) {
  ...
  /* I think this is wrong.  You want shut server_sock not netlib_control, as
     shutdown_control() does. (pavel 5-Feb-1998)*/
#if 0
  shutdown_control(); 
#endif
...
short int	port;
{
  ...
  if ((addr = inet_addr(hostname)) == -1) {
    ...
    if ((addr =  hostGetByName (hostname)) == NULL) {
	/* we have no idea what it is */
	fprintf(where,
		"establish_control: could not resolve the destination %s\n",
		hostname);
...
#elif defined(VXWORKS)
/* fill in something here to get nodename, release level... */
#else
...
#else
#ifdef VXWORKS
    /* For the moment, punt on this.  I'm sure there's some sort of resource
       measurement API in VxWorks, but I don't yet know what it is...
       (pavel 7-Jan-1998) */
#else
    cpu_method = TIMES;
    times(&times_data1);
